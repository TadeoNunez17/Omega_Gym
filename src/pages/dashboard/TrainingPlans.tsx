...
const EXERCISES_DB: Record<number, any[] | null> = {
  0: [
    { name:'Press de banca plano',      muscle:'Pecho',         sets:4, reps:10, rest:90,  note:'Barra olÃ­mpica' },
    ...
    { name:'Fondos en paralelas',       muscle:'TrÃ­ceps',       sets:3, reps:12, rest:60,  note:'' },
    { name:'ExtensiÃ³n trÃ­ceps polea',   muscle:'TrÃ­ceps',       sets:4, reps:12, rest:60,  note:'' },
  ],
  1: [
    { name:'JalÃ³n al pecho',            muscle:'Espalda',       sets:4, reps:10, rest:90,  note:'Agarre ancho' },
    ...
    { name:'Curl de bÃ­ceps barra',      muscle:'BÃ­ceps',        sets:3, reps:12, rest:60,  note:'' },
    { name:'Curl martillo',             muscle:'BÃ­ceps',        sets:3, reps:12, rest:60,  note:'' },
  ],
  2: [
    { name:'Sentadilla libre',          muscle:'CuÃ¡driceps',    sets:4, reps:8,  rest:120, note:'Prioridad tÃ©cnica' },
    { name:'Prensa de piernas',         muscle:'CuÃ¡driceps',    sets:3, reps:12, rest:90,  note:'' },
    { name:'ExtensiÃ³n de cuÃ¡driceps',   muscle:'CuÃ¡driceps',    sets:3, reps:15, rest:60,  note:'' },
  ...
  3: [
    ...
    { name:'PÃ¡jaro con mancuernas',     muscle:'Deltoides post',sets:3, reps:15, rest:60,  note:'' },
  ...
  4: [
    { name:'Peso muerto convencional',  muscle:'Espalda baja',  sets:3, reps:8,  rest:120, note:'TÃ©cnica perfecta' },
    { name:'Dominadas asistidas',       muscle:'Espalda',       sets:3, reps:8,  rest:90,  note:'' },
    { name:'Hip thrust',               muscle:'GlÃºteo',         sets:4, reps:12, rest:60,  note:'' },
...
const INITIAL_PLANS: Plan[] = [
  { id:1, name:'Fuerza A',      desc:'Pecho, espalda, piernas, hombros y core. Hipertrofia intermedia.',   type:'assigned', trainer:'Miguel Torres', member:'Carlos RamÃ­rez',  memberAv:'CR', avClass:'av-0', days:5, exercises:EXERCISES_DB },
  { id:2, name:'Cardio Plus',   desc:'Programa cardiovascular de 6 semanas. Ideal para pÃ©rdida de peso.',  type:'assigned', trainer:'Diana Salazar',  member:'SofÃ­a LÃ³pez',     memberAv:'SL', avClass:'av-1', days:5, exercises:EXERCISES_DB },
  { id:3, name:'Fuerza B',      desc:'Variante de Fuerza A con Ã©nfasis en tren inferior y compuestos.',    type:'assigned', trainer:'Miguel Torres', member:'Ana GutiÃ©rrez',   memberAv:'AG', avClass:'av-3', days:5, exercises:EXERCISES_DB },
  { id:4, name:'Movilidad',     desc:'Rutina de flexibilidad y movilidad articular. 5 dÃ­as semanales.',    type:'assigned', trainer:'Diana Salazar',  member:'Valeria Cruz',    memberAv:'VC', avClass:'av-5', days:5, exercises:EXERCISES_DB },
  { id:5, name:'Principiante General', desc:'Plan base para nuevos miembros. AdaptaciÃ³n y tÃ©cnica.',       type:'template', trainer:'Miguel Torres', member:null,              memberAv:null, avClass:'',    days:4, exercises:EXERCISES_DB },
  { id:6, name:'Hipertrofia Avanzada', desc:'Plan de 5 dÃ­as para atletas con experiencia mayor a 2 aÃ±os.', type:'draft',   trainer:'Admin',         member:null,              memberAv:null, avClass:'',    days:5, exercises:EXERCISES_DB },
];
        ...
        <div style={{
          ...
          <div style={{ padding: 12, borderBottom: '1px solid var(--border)' }}>
            <div style={{ position: 'relative' }}>
              ...
              <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Buscar plan o miembroâ€¦"
                style={{
                ...
                <div key={p.id} onClick={() => { setSelectedId(p.id); setSelectedDay(0); }}
                  ...
                  <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 8 }}>
                    <div>
                      ...
                      <div style={{ fontSize: 11, color: 'var(--text-3)', marginTop: 2 }}>{p.member ? `â†’ ${p.member}` : p.trainer}</div>
                    </div>
                    <span style={{
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                    ...
                    <ChipSm icon="calendar" text={`${p.days} dÃ­as / semana`} />
                  </div>
                </div>
              );
            })}
                  ...
                  if (exs === null) {
                    return (
                      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: 60, gap: 10, textAlign: 'center' }}>
                        ...
                        <div style={{ fontSize: 14, fontWeight: 600 }}>DÃ­a de descanso</div>
                    ...
                    <>
                      <div style={{
                        ...
                        <span style={{ fontSize: 11, color: 'var(--text-3)', textTransform: 'uppercase', letterSpacing: '0.06em' }}>
                          {DAY_NAMES[selectedDay]} Â· {exs.length} ejercicio{exs.length !== 1 ? 's' : ''}
                        </span>
                        <button style={{
            ...
            <div style={{ padding: 24, display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
                <FormGroup full label="Nombre del plan *">
                  <input value={fName} onChange={e => setFName(e.target.value)} placeholder="Ej. Fuerza A â€” Intermedio"
                    style={inputStyle} />
                </FormGroup>
                <FormGroup full label="DescripciÃ³n">
                  <textarea value={fDesc} onChange={e => setFDesc(e.target.value)} placeholder="Objetivo del plan, observaciones generalesâ€¦"
                ...
                {fType === 'assigned' && (
                  <FormGroup full label="Asignar a miembro">
                    <select value={fMember} onChange={e => setFMember(e.target.value)} style={{ ...inputStyle, cursor: 'pointer' }}>
                      <option value="">â€” Seleccionar miembro â€”</option>
                      <option>Carlos RamÃ­rez</option>
                      <option>SofÃ­a LÃ³pez</option>
                      <option>Ana GutiÃ©rrez</option>
                      <option>Paola Rivas</option>
                      <option>HÃ©ctor GÃ³mez</option>
                    </select>
                  </FormGroup>
                )}
              </div>
