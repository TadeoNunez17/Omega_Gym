...
export default function TrainerMembersPage() {
  ...
  const fetchMembers = useCallback(async () => {
    ...
    try {
      ...
      const nonAdmin = result.data.map(toMember).filter((m) => m.role !== 'admin');
      setMembers(nonAdmin);
      setTotal(nonAdmin.length);
      setStats(s);
    } catch (e: any) {
      setError(e.message);
    } finally {
    ...
    <>
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title="Nuevo miembro" className="max-w-[400px]" icon={<IconPlus width="16" height="16" />}>
        <div className="flex flex-col gap-4">
          ...
          <div className="flex items-start gap-2.5 p-3 rounded-sm bg-green-bg border border-green/20 text-[11px] text-green-text leading-relaxed">
            <svg className="mt-0.5 shrink-0" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5">
              <circle cx="12" cy="12" r="10"/><line x1="12" y1="16" x2="12" y2="12"/><line x1="12" y1="8" x2="12.01" y2="8"/>
            </svg>
            <span>Perfil pendiente — vincúlalo cuando el usuario se registre.</span>
          </div>
        </div>
              ...
              {detailMode === 'edit' && (
                <div className="border-t border-border pt-5">
                  ...
                  <div className="border border-accent/15 bg-accent/5 rounded-sm p-4">
                    ...
                    <p className="text-[10px] text-text-3/60 mt-2">Identifica al miembro en pre-registro. Visible para admin y entrenadores.</p>
                  </div>
                </div>
              )}
            </>
      ...
      <Modal open={unlinkTarget !== null} onClose={() => setUnlinkTarget(null)} title="Desvincular cuenta" className="max-w-[400px]" icon={
