              ...
              return (
                <div key={p.id} className={`flex items-start sm:items-center gap-3 px-[18px] py-3.5 animate-slide-up ${staggerClass(i)} ${i < plans.length - 1 ? 'border-b border-border' : ''}`}>
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    ...
                    <button className="w-10 sm:w-7 h-10 sm:h-7 [&>svg]:w-[18px] [&>svg]:h-[18px] sm:[&>svg]:w-[13px] sm:[&>svg]:h-[13px] rounded-[var(--radius-sm)] bg-transparent border border-border text-text-3 cursor-pointer flex items-center justify-center hover:bg-surface2 transition-colors">
