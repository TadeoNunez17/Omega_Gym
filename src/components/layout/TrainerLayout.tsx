...
import { trainerNavItems, trainerIcons } from './trainer-sidebar-config'
...
const trainerBottomIcons: Record<string, string> = {
  ...trainerIcons,
...
export function TrainerLayout() {
  ...
  const initials = user ? user.full_name.split(' ').map(w => w[0]).join('').slice(0, 2).toUpperCase() : 'TR'

  const collapsedItems = trainerNavItems.flatMap(s => s.items)
  ...
  return (
    <div className="min-h-screen bg-bg">
            ...
            return (
              <Link key={item.href} to={item.href}
                ...
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8"
                  dangerouslySetInnerHTML={{ __html: trainerIcons[item.icon] || '' }} />
              </Link>
            )
          })}
        </div>
      ...
      <BottomNav items={trainerBottomItems} icons={trainerBottomIcons} onSettings={() => setShowSettings(true)} />
    </div>
  )
}
