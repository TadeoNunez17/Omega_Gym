...
interface Props {
  ...
  onSelect: (exercise: { id: string; name: string; muscle: string | null; gif_url: string | null; reference_link: string | null }) => void
  onCustomMode: () => void
  placeholder?: string
}

export function ExercisePicker({ value, exerciseId, onSelect, onCustomMode, placeholder }: Props) {
  ...
  const [isCustom, setIsCustom] = useState(!exerciseId && !!value)
  ...
  }, [query, selectedCategory, search, isCustom])

  useEffect(() => {
    if (!open || isCustom) return
    search(query, selectedCategory)
  }, [open, search, query, selectedCategory, isCustom])
  ...
  return (
    <div ref={containerRef} className="relative">
      <label className="text-xs font-medium text-text-2 block mb-1.5">Ejercicio</label>

      {/* Toggle catalog/custom */}
      <div className="flex gap-1.5 mb-2">
        <button
          type="button"
          onClick={() => { if (isCustom) handleCustomToggle() }}
          className={`px-2.5 py-1 rounded-full text-[10px] font-medium cursor-pointer transition-all duration-150 border ${
            !isCustom
              ? 'bg-accent text-black border-accent'
              : 'bg-transparent text-text-3 border-border hover:bg-surface2 hover:text-text-2'
          }`}>
          Catalogo
        </button>
        <button
      ...
      {isCustom ? (
        <input
          type="text"
          value={query}
          onChange={e => { setQuery(e.target.value); onSelect({ id: '', name: e.target.value, muscle: null, gif_url: null, reference_link: null }) }}
          placeholder={placeholder || 'Ej. Press banca'}
          className="w-full bg-surface2 border border-border2 text-text font-sans text-xs px-2.5 py-1.5 rounded-sm outline-none placeholder:text-text-3 focus:border-accent transition-colors"
        />
      ) : (
        <>
          {/* Search input */}
          <div className="relative">
            ...
            <input
              ...
              className="w-full bg-surface2 border border-border2 text-text font-sans text-xs pl-8 pr-2.5 py-1.5 rounded-sm outline-none placeholder:text-text-3 focus:border-accent transition-colors"
            />
          </div>
                ...
                results.map(ex => (
                  <button
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="text-[11px] font-medium text-text truncate capitalize">{ex.name}</div>
