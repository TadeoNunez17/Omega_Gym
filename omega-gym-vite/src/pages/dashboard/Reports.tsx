...
const REPORTS: Report[] = [
  { name: 'Miembros por plan', description: 'DistribuciÃ³n de miembros segÃºn tipo de membresÃ­a contratada', icon: 'ðŸ“Š', color: 'var(--blue)', rows: 12 },
  { name: 'Asistencia mensual', description: 'Check-ins diarios promedio por mes', icon: 'ðŸ“ˆ', color: 'var(--green)', rows: 14 },
  { name: 'Ingresos por periodo', description: 'RecaudaciÃ³n total filtrada por rango de fechas', icon: 'ðŸ’°', color: 'var(--accent-text)', rows: 8 },
  { name: 'Renovaciones', description: 'Tasa de retenciÃ³n y membresÃ­as renovadas vs. canceladas', icon: 'ðŸ”„', color: 'var(--amber)', rows: 10 },
  { name: 'Morosidad', description: 'Miembros con pagos pendientes y dÃ­as de retraso', icon: 'âš ï¸', color: 'var(--red)', rows: 6 },
  { name: 'Planes populares', description: 'Planes de entrenamiento mÃ¡s asignados este mes', icon: 'ðŸ‹ï¸', color: 'var(--purple, #a78bfa)', rows: 9 },
];
    ...
    <>
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 13, color: 'var(--text-3)' }}>
          Panel
          <span style={{ fontSize: 10 }}>â€º</span>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <div style={{ display: 'flex', gap: 4, background: 'var(--surface)', borderRadius: 'var(--radius-sm)', padding: 2, border: '1px solid var(--border)' }}>
            {(['month', 'quarter', 'year'] as const).map((p) => (
              ...
              >
                {p === 'month' ? 'Mes' : p === 'quarter' ? 'Trimestre' : 'AÃ±o'}
              </button>
            ))}
          </div>
      ...
      <div style={{ padding: 28, flex: 1 }}>
        <div style={{ marginBottom: 24 }}>
          ...
          <div style={{ fontSize: 13, color: 'var(--text-2)', marginTop: 4 }}>
            AnalÃ­tica y mÃ©tricas clave del gimnasio
          </div>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12, marginBottom: 20 }}>
          {[
            ...
            { label: 'Tasa retenciÃ³n', value: '89%', change: '-2%', up: false, color: 'amber', sub: 'Ãšltimos 30 dÃ­as' },
          ].map((m) => (
          ...
          <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius)', padding: 20 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
              <div>
                ...
                <div style={{ fontSize: 11, color: 'var(--text-3)', marginTop: 2 }}>Enero â€“ Mayo 2026</div>
              </div>
