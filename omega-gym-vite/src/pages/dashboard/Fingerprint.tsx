...
const MEMBERS: Member[] = [
  { name:'Carlos RamÃ­rez',  email:'carlos@correo.com',  av:'CR', avC:'av-0', hasHuella:true  },
  { name:'SofÃ­a LÃ³pez',     email:'sofia@correo.com',   av:'SL', avC:'av-1', hasHuella:false },
  { name:'Ana GutiÃ©rrez',   email:'ana@correo.com',     av:'AG', avC:'av-3', hasHuella:true  },
  ...
  { name:'Roberto FÃ©lix',   email:'rober@correo.com',   av:'RF', avC:'av-0', hasHuella:false },
  ...
  { name:'HÃ©ctor GÃ³mez',    email:'hector@correo.com',  av:'HG', avC:'av-4', hasHuella:false },
];
    ...
    <div style={{ padding: 28, flex: 1, display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      <header style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 13, color: 'var(--text-3)' }}>
          Panel <span style={{ color: 'var(--text-2)' }}>â€º</span> <span style={{ color: 'var(--text-2)' }}>Registro de Huella Dactilar</span>
        </div>
        <span style={{
          ...
          Lector: <span style={{ color: 'var(--green-text)' }}>â— Conectado</span>
        </span>
      </header>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '320px 1fr', gap: 20, alignItems: 'start' }}>
        ...
        <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius)', overflow: 'hidden' }}>
          ...
          <div style={{ padding: 10, borderBottom: '1px solid var(--border)', position: 'relative' }}>
            ...
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Buscar miembroâ€¦"
              style={{
              ...
              return (
                <div key={m.name} onClick={() => selectMember(m)}
                  ...
                  <div style={{ flexShrink: 0 }}>
                    {m.hasHuella ? (
                      <span style={{ fontSize: 10, background: 'var(--green-bg)', color: 'var(--green-text)', padding: '2px 8px', borderRadius: 100, fontWeight: 500 }}>âœ“ Huella</span>
                    ) : (
            ...
            <>
              ...
              {selected.hasHuella && (
                <div style={{
                  ...
                  <span>Este miembro ya tiene una huella registrada. Continuar <strong>reemplazarÃ¡</strong> el template actual en <code style={{ background: 'rgba(0,0,0,0.2)', padding: '1px 5px', borderRadius: 3 }}>profiles.huella_template</code>.</span>
                </div>
              )}
              ...
              <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius)', overflow: 'hidden' }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 20px', borderBottom: '1px solid var(--border)' }}>
                  <div>
                    ...
                    <div style={{ fontSize: 11, color: 'var(--text-3)', marginTop: 2 }}>
                      {sensorState === 'scanning' ? 'Mantenlo quieto hasta escuchar el pitido' : sensorState === 'success' ? 'Template registrado correctamente' : 'Coloca el dedo sobre el lector cuando estÃ©s listo'}
                    </div>
                  </div>
                </div>
                ...
                <div style={{ padding: 32, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 24 }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 0, width: '100%' }}>
                    {['Inicio','Escaneo','VerificaciÃ³n','Guardado'].map((label, i) => {
                  ...
                  <div style={{
                    ...
                    <div style={{
                      ...
                      {sensorState === 'idle' ? 'Esperandoâ€¦' : sensorState === 'scanning' ? (scans < 3 ? 'Coloca el dedoâ€¦' : 'Procesandoâ€¦') : sensorState === 'success' ? 'Â¡Huella guardada!' : 'Error'}
                    </div>
                  </div>
      ...
      <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius)', overflow: 'hidden' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 20px', borderBottom: '1px solid var(--border)' }}>
          <div>
            ...
            <div style={{ fontSize: 11, color: 'var(--text-3)', marginTop: 2 }}>Ãšltimas huellas capturadas en esta sesiÃ³n</div>
          </div>
          <span style={{ fontSize: 12, color: 'var(--text-3)' }}>{history.length} registro{history.length !== 1 ? 's' : ''}</span>
        </div>
        {history.length === 0 ? (
          <div style={{ padding: 40, textAlign: 'center', color: 'var(--text-3)', fontSize: 13 }}>AÃºn no se han registrado huellas en esta sesiÃ³n.</div>
        ) : (
          history.map((h, i) => {
